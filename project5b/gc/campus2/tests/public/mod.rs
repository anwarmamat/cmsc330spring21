extern crate campus2 as campus;
...
fn public_test_add_get_turtle() {
    ...
    assert_eq!(u.borrowed_turtle().name().as_str(), TESTUDO);

    let yertle = new_yertle();
    campus.add_turtle(yertle);

    let t0 = campus.get_turtle(0);
    assert_eq!(t0.borrowed_turtle().name().as_str(), TESTUDO);

    let t1 = campus.get_turtle(1);
    assert_eq!(t1.borrowed_turtle().name().as_str(), YERTLE);
}
...
fn new_testudos() {
    ...
    for i in 0..10 {
        ...
        assert_eq!(t.borrowed_turtle().name().as_str(), TESTUDO);
    }
}
...
fn public_test_fastest_walker() {
    ...
    match campus.fastest_walker() {
        None => assert!(false),
        Some(f) => assert_eq!(f.borrowed_turtle().name().as_str(), TESTUDO),
    }
    ...
    match campus.fastest_walker() {
        None => assert!(false),
        Some(f) => assert_eq!(f.borrowed_turtle().name().as_str(), YERTLE),
    }
...
fn public_test_breed_turtles() {
...

    {
        ...
        let testudo_r = testudo.borrowed_turtle();
        let yertle = campus.get_turtle(1);
        let yertle_r = yertle.borrowed_turtle();
    ...
    let (testudo_flavor, yertle_flavor) = {
        ...
        let child_r = child.borrowed_turtle();
    ...
    for i in 0..1000 {
        campus.breed_turtles(0, 1, String::from("Mack"));
        let new_flavor = campus.get_turtle(i+3).borrowed_turtle().favorite_flavor();
...
fn public_test_teach_children() {
    ...
    campus.add_turtle(new_yertle());
    campus.breed_turtles(0, 1, String::from("Mack"));
    let child_walking_speed = campus.get_turtle(2).borrowed_turtle().walking_speed();
    
    campus.get_turtle(0).borrowed_turtle().teach_children();
    let new_child_walking_speed = campus.get_turtle(2).borrowed_turtle().walking_speed();
